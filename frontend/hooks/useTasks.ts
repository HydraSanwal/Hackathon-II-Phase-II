...
export function useTasks(): UseTasksReturn {
  ...
  const updateTask = useCallback(async (id: number, data: TaskUpdateData): Promise<Task> => {
    ...
    try {
      // Pass the data as is since the API client now accepts optional title
  ...
  const patchTask = useCallback(async (id: number, data: Partial<TaskUpdateData>): Promise<Task> => {
    setState((prev) => ({ ...prev, error: null }));
    try {
      // Ensure title is never undefined when provided
      const validatedData: Partial<{ title: string; description?: string; is_completed: boolean }> = {};
      ...
      if ('is_completed' in data) {
        validatedData.is_completed = data.is_completed!;
      }
      
      const updatedTask = await taskApi.patchTask(id, validatedData);
      setState((prev) => ({
        ...prev,
        items: prev.items.map((task) =>
          task.id === id ? updatedTask : task
